...
video_list_long = get_video_list(url =  config['URL_SEARCH'], api_key = config['API_KEY'], channel_id= config['CHANNEL_ID'],published_after= config['PUBLISHED_AFTER'], published_before = config['PUBLISHED_BEFORE'],  video_duration= config['VIDEO_DURATION_LONG'])
video_list_medium = get_video_list(url =  config['URL_SEARCH'], api_key = config['API_KEY'], channel_id= config['CHANNEL_ID'], published_after= config['PUBLISHED_AFTER'], published_before= config['PUBLISHED_BEFORE'], video_duration= config['VIDEO_DURATION_MEDIUM'])
video_list = video_list_long + video_list_medium
with open(config['FILE_VIDEO_LIST'], 'w') as file: 
    json.dump(video_list_long, file)

print('3. Saving of videos statistics list has been started.')
video_statistics, category_list = get_video_statistics(url = config['URL_VIDEOS'], channelId = config['CHANNEL_ID'], videoList = video_list, api_key= config['API_KEY'])
with open(config['PATH_VIDEO_STATISTICS_LIST'], 'w') as file:
    json.dump(video_statistics, file)
    
print('4. Saving of videos categories list has been started.')
video_categories = get_video_categories(url=config['URL_VIDEO_CATEGORIES'], api_key= config['API_KEY'], category_ids= category_list)
with open(config['PATH_VIDEOS_CATEGORIES'], 'w') as file:
